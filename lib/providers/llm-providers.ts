...
export const llmProviders: ProviderConfig[] = [
  {
    ...
    parseResponse: (data) => ({
      models: Array.isArray(data?.data) ? data.data.map((m: Record<string, unknown>) => m.id).filter(Boolean) : undefined,
  ...
  },
  {
    ...
    parseResponse: (data) => ({
      models: Array.isArray(data?.models) ? data.models.map((m: Record<string, unknown>) => m.name).filter(Boolean) : undefined,
